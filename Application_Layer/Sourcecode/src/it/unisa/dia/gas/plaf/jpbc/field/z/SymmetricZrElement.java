...
public class SymmetricZrElement extends AbstractZElement {
    ...
    public SymmetricZrElement(SymmetricZrElement zrElement) {
        ...
        this.order = zrElement.abstractField.getOrder();
        this.halfOrder = ((SymmetricZrField) zrElement.abstractField).halfOrder;
        set(zrElement);
    }
...
//        this.value = value.multiply(BigIntegerUtils.TWO).mod(order);
    ...
    public SymmetricZrElement setToRandom() {
        this.value = new BigInteger(order.bitLength(), abstractField.getRandom()).mod(order);
    ...
    public int setFromBytes(byte[] source, int offset) {
        byte[] buffer = Arrays.copyOf(source, offset, abstractField.getLengthInBytes());
...
//        value = value.modPow(BigIntegerUtils.TWO, order);
    ...
    public SymmetricZrElement halve() {
        value = value.multiply(((ZrField) abstractField).twoInverse).mod(order);
    ...
    public SymmetricZrElement sqrt() {
        ...
        Element e0 = abstractField.newElement();
        Element nqr = abstractField.getNqr();
    ...
    public byte[] toBytes() {
        ...
        if (bytes.length > abstractField.getLengthInBytes()) {
            // strip the zero prefix
            if (bytes[0] == 0 && bytes.length == abstractField.getLengthInBytes() + 1) {
                // Remove it
                bytes = Arrays.copyOfRange(bytes, 1, bytes.length);
            } else
                throw new IllegalStateException("result has more than FixedLengthInBytes.");
        } else if (bytes.length < abstractField.getLengthInBytes()) {
            byte[] result = new byte[abstractField.getLengthInBytes()];
            System.arraycopy(bytes, 0, result, abstractField.getLengthInBytes() - bytes.length, bytes.length);
            return result;
        }
        return bytes;
    }
